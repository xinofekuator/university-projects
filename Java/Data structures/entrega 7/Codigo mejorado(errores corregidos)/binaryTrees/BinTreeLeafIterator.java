...
public class BinTreeLeafIterator<E> implements Iterator<E> {
	...
	private Position<E> nextLeaf(Position<E> p){
		...
		while(this.tree.isInternal(aux)){
			if(this.tree.hasLeft(aux))
	...
	private void avanzar(){
		...
		while(actual!=null && !terminado){
			...
			if(this.tree.root()==aux || (this.tree.parent(aux)==this.tree.root() && (this.tree.right(this.tree.root())==aux) || !this.tree.hasRight(this.tree.root()))){
				actual=null;
				terminado=true;
			}
